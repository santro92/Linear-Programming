function [S,eflag] = sudokuEngine(B,n)
...
Nsquare = n * n;
if isequal(size(B),[Nsquare,Nsquare]) % 9-by-9 clues
    % Convert to 81-by-3
...
if sum([any(B ~= round(B)),any(B < 1),any(B > Nsquare)]) % enforces entries 1-9
    error('Entries must be integers from 1 to 9')
end

%% The rules of Sudoku:
N = Nsquare^3; % number of independent variables in x, a 9-by-9-by-9 array
M = 4*Nsquare^2; % number of constraints, see the construction of Aeq
...
lb = zeros(Nsquare,Nsquare,Nsquare); % an initial zero array
...
if eflag > 0 % good solution
    x = reshape(x,Nsquare,Nsquare,Nsquare); % change back to a 9-by-9-by-9 array
