function [bas1,nonbas1,objval,isFinal,isUnbounded] = pivotT(A,b,c,bas,nonbas)
...
function [bas1,nonbas1,objval,isFinal,isUnbounded] = pivot(A,b,c,bas,nonbas)
...
enteringIndex = find(nonbas == min(nonbas(chat > 0)));
fprintf(1,'Entering Variable: %d \n',min(nonbas(enteringIndex)));
...
for i = 1:m
    if (Aj(i,1) < 0)
          ll = -bhat(i,1)/Aj(i,1);
        if (ll < leavingLim - 1E-08 || ( ll >= leavingLim - 1E-08 && ll <= leavingLim + 1E-08 && bas(i) <= bas(leavingIndex) ))
            leavingLim = ll;
            leavingIndex = i;
        end
    end
